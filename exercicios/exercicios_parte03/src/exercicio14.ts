...
async function posicaoX(xA:number, xB:number) {
    const posicaoX = Math.pow((xB - xA), 2)
    return posicaoX
}

async function posicaoY(yA:number, yB:number) {
    const posicaoY = Math.pow((yB - yA), 2)
    return posicaoY
}

async function distanciaEntrePontos(xA:number, xB:number, yA:number, yB:number) {
